  ...
  module Schema
    module Validation
      ...
      def self.validate_params_against_query_definition(query : GraphQL::Language::OperationDefinition, params = nil)
        ...
        query.variables.as(Array(GraphQL::Language::VariableDefinition)).each do |variable|
          # param given with query
          if (param = params[variable.name]?)
            errors = field_definition_rejects_value?(variable.type.as(GraphQL::Language::Type), param)
          ...
          elsif !variable.type.is_a?(GraphQL::Language::NonNullType)
